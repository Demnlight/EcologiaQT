...
class API {
   ...
   public:
    ...
    struct answerForFirstPractice {
        double M_L_PM2_5;  // Выброс загрязняющего вещества движущимся автотранспортным потоком (г/с)
        double M_L_PM10;   // Выброс загрязняющего вещества движущимся автотранспортным потоком (г/с)
        double M_L_NO2;    // Выброс загрязняющего вещества движущимся автотранспортным потоком (г/с)
        double M_L_O3;     // Выброс загрязняющего вещества движущимся автотранспортным потоком (г/с)
        double M_L_SO2;    // Выброс загрязняющего вещества движущимся автотранспортным потоком (г/с)
        double M_L_CO;     // Выброс загрязняющего вещества движущимся автотранспортным потоком (г/с)
    };
    ...
    static answerForFirstPractice calculateAnswerForFirstPractice(Variant variant) {
        ...
        // Перевод в string
        answer.M_L_PM2_5 = (M_L_PM2_5);
        answer.M_L_PM10 = (M_L_PM10);
        answer.M_L_NO2 = (M_L_NO2);
        answer.M_L_O3 = (M_L_O3);
        answer.M_L_SO2 = (M_L_SO2);
        answer.M_L_CO = (M_L_CO);
    ...
    void compareAnswerForFirstPractice(answerForFirstPractice userAnswer) {
        ...
        auto checkDelta = [](double first, double second, double delta = 0.0001) -> bool
        {
            return abs(first - second) < delta;
        };

        currentResult += checkDelta(rightAnswer.M_L_PM2_5, userAnswer.M_L_PM2_5) ? 1 : 0;
        currentResult += checkDelta(rightAnswer.M_L_PM10 , userAnswer.M_L_PM10) ? 1 : 0;
        currentResult += checkDelta(rightAnswer.M_L_NO2 , userAnswer.M_L_NO2) ? 1 : 0;
        currentResult += checkDelta(rightAnswer.M_L_O3 , userAnswer.M_L_O3) ? 1 : 0;
        currentResult += checkDelta(rightAnswer.M_L_SO2 , userAnswer.M_L_SO2) ? 1 : 0;
        currentResult += checkDelta(rightAnswer.M_L_CO , userAnswer.M_L_CO) ? 1 : 0;
